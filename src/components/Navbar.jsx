      ...
      <motion.nav
        ...
        className={`fixed w-full z-50 transition-all duration-300 ${
          isScrolled
            ...
            : 'bg-transparent text-gray-900 dark:bg-gray-900 dark:text-white'
        }`}
      >
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navItems.map((item) => (
                <motion.a
                  key={item.name}
                  href={item.href}
                  className="text-white hover:text-yellow-400 dark:text-gray-300 dark:hover:text-yellow-400 transition-colors duration-200"
              ...
              <div className="mt-8 space-y-4">
                {navItems.map((item) => (
                  <motion.a
                    key={item.name}
                    href={item.href}
                    className="block text-gray-600 hover:text-teal-400 dark:text-gray-300 dark:hover:text-teal-400 transition-colors duration-200 py-2"
                    whileHover={{ x: 10 }}
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    {item.name}
